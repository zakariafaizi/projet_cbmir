...
def get_neighbors(train, test_row, num_neighbors):
    ...
    for train_row in train:
        dist = euclidean_distance(test_row, pd.to_numeric(train_row[1:27]))
...
bit_file = '../datasets/Outex_Bit_Haralick.csv'
...
for neighbor in neighbors:
    print("Result Image's name : ", neighbor[0], "at folder : ", neighbor[28])
    imgpath = findimage(neighbor[28], neighbor[0])
    ...
    plt.axis('off')
    ax.set_title(str(neighbor[0]) + " class "+ str(neighbor[28]), fontsize=7) # gives title to each image
    plt.margins(0, 0)
    i+= 1


plt.savefig(output_path+'output_bit_haralick.png', bbox_inches='tight',pad_inches = 1)  #to save an image containing all the outputs
img_color = cv2.imread(output_path+'output_bit_haralick.png', 1)  # 1: Color image. 1 is optional.
